...
- After CI succeeds on `develop`, dispatch `Deploy BFF to Cloud Run (dev)` with the exact candidate SHA. The completed DEV run automatically triggers `BFF production-promotion-ready`; if that trigger is unavailable, manually dispatch the readiness workflow on `develop` with the same SHA and exact DEV run ID. It performs no provider or Production operation and uploads `bff-production-promotion-ready-<SHA>` only after the shared receipt and repository-owned rollback fixtures pass. Before the ordinary non-force `develop` to `main` fast-forward, record that readiness artifact; then dispatch production with the exact successful `main` DEV run ID and candidate SHA. The release workflow revalidates that run and receipt with the same production-owned validator before its existing Production approval, rollback capture, read-back, mutation, evidence, and tagging gates.
