...
- After CI succeeds on `develop`, dispatch `Deploy BFF to Cloud Run (dev)` with the exact candidate SHA. Its successful DEV deployment and artifact feed the read-only downstream job named exactly `production-promotion-ready`; that job runs only when `github.ref == refs/heads/develop`, verifies the candidate is still the exact current canonical `develop` head, and carries the readiness check on that same SHA. If `develop` advances before the job runs, readiness fails closed. Before an ordinary non-force `develop` to `main` fast-forward, require both successful `main-fast-forward-eligible` and successful `production-promotion-ready` checks for the selected SHA; a prior successful readiness check remains usable for an older SHA only when it was earned while that SHA was the canonical `develop` head. No branch protection is mutated by this change; after canonical merge and the first successful check, add the stable readiness name as required protection. Then dispatch production with the explicit exact successful `main` DEV run ID and candidate SHA. The release workflow revalidates that run and receipt with the same production-owned validator before its existing Production approval, rollback capture, read-back, mutation, evidence, and tagging gates.
