...
func TestDockerfileCopiesReviewedQueryConfigWithoutBakingEnv(t *testing.T) {
	...
	if !strings.Contains(dockerfile, "COPY --from=build /src/configs/query /app/configs/query") {
		t.Fatal("Dockerfile does not copy configs/query into the runtime image")
	}
